...
**`-M`** 使 `fsck_msdosfs` 在检查 FAT32 文件系统时不使用 [mmap(2)](../sys/mmap.2.md)。此选项主要用于调试目的，通常不需要。当 `fsck_msdosfs` 执行 [mmap(2)](../sys/mmap.2.md) 失败，或指定了 `-M` 时，会自动回退到使用 4 MiB 的简单 LRU 缓存。
