...
命名扩展属性允许将额外的元数据与表示文件和目录的 vnode 关联。这些额外数据的语义为 "name=value" 对，其中名称可以被定义或未定义，如果被定义，则关联零个或多个字节的任意二进制数据。扩展属性名存在于一个命名空间集合中；对扩展属性的每个操作都要求提供该操作所引用的命名空间。如果同一名称出现在多个命名空间中，与这些名称关联的扩展属性会被独立存储和操作。以下两个命名空间是通用的，但单个文件系统可以实现额外的命名空间，也可以不实现这些命名空间：`EXTATTR_NAMESPACE_USER`、`EXTATTR_NAMESPACE_SYSTEM`。这些属性的语义意图如下：用户属性数据根据与文件或目录中数据相关的常规自主和强制保护进行保护；系统属性数据的保护要求具有适当权限才能直接访问或操作这些属性。默认情况下，[jail(8)](../man8/jail.8.md) 中的进程无法访问系统属性数据，除非指定了 `allow.extattr` 配置参数。
...
此外，该接口未提供检索当前可用属性集合的机制；有人建议提供 `NULL` 属性名时应返回所传入文件或目录已定义属性列表，但目前尚未实现。
