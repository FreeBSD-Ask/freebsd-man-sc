...
用于从微序列返回。此指令是必需的。这是微序列器检测微序列结束的唯一方式。返回码通过 ppb_MS_microseq() 的 (int *) 参数所指向的整数传回。
...
用于触发并行端口。此微指令旨在非常高效地控制并行端口。触发寄存器即写入数据、等待片刻、写入数据、等待片刻……这允许向端口写入魔序列。参数：
