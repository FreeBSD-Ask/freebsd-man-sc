...
```c
ITYPE Q_IRAWMASK(QTYPE q);
ITYPE Q_GIRAW(QTYPE q);
ITYPE Q_GIABSVAL(QTYPE q);
ITYPE Q_GIVAL(QTYPE q);
QTYPE Q_SIVAL(QTYPE q, ITYPE iv);
```

## 描述

`Q_IRAWMASK` 返回 `q` 的整数数据位所对应的特定位掩码。

`Q_GIRAW` 返回 `q` 的原始掩码整数数据位。

`Q_GIABSVAL` 与 `Q_GIVAL` 分别返回 `q` 的整数数据位的绝对值和实际值。

`Q_SIVAL` 将 `q` 的整数数据位设置为值 `iv`。
...
`Q_IRAWMASK`、`Q_GIRAW`、`Q_GIABSVAL` 和 `Q_GIVAL` 以与 `q` 相同底层 ITYPE 的整数形式返回各自对应的值。
