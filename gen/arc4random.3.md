...
此函数系列提供的数据质量高于 [rand(3)](rand.3.md)、[random(3)](random.3.md) 和 [rand48(3)](rand48.3.md) 中所述的函数。

几乎所有随机数使用场景都鼓励使用这些函数，因为其他接口在质量、可移植性、标准化或可用性方面存在不足。这些函数几乎可以在所有编程环境中调用，包括 [pthread(3)](pthread.3.md) 和 [chroot(2)](../man2/chroot.2.md)。

高质量的 32 位伪随机数生成速度非常快。每次调用时，都使用加密伪随机数生成器来生成新的结果。一个数据池供进程中的所有消费者使用，因此程序流下的使用可以作为额外的搅拌。子系统会定期使用 [getentropy(3)](getentropy.3.md) 从内核 [random(4)](../man4/random.4.md) 子系统重新播种，并在 [fork(2)](../man2/fork.2.md) 时也会重新播种。

`arc4random` 函数返回单个 32 位值。`arc4random` 函数返回 0 到 (2\*\*32)-1 范围内的伪随机数，因此其范围是 [rand(3)](rand.3.md) 和 [random(3)](random.3.md) 的两倍。
...
[rand(3)](rand.3.md), [rand48(3)](rand48.3.md), [random(3)](random.3.md)
