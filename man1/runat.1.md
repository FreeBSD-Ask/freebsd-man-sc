...
如果 `file` 没有命名属性目录，将创建一个空目录。如果应用程序需要确定 `file` 是否存在命名属性，可以使用 [pathconf(2)](../man2/pathconf.2.md) 配合名称 `_PC_HAS_NAMEDATTR`。这不会在 `file` 不存在命名属性目录时创建空的命名属性目录。
...
[sh(1)](sh.1.md), fchdir(2), [pathconf(2)](../man2/pathconf.2.md), [open(2)](../man2/open.2.md), [named_attribute(7)](../man7/named_attribute.7.md)
