...
活动套接字的默认显示包括本地和远程地址、发送和接收队列大小（字节）、协议以及协议的内部状态。如果套接字地址指定了网络但无特定主机地址，地址格式为 “host.port” 或 “network.port”。如果已知，主机和网络地址分别根据 [hosts(5)](../man5/hosts.5.md) 和 [networks(5)](../man5/networks.5.md) 数据库符号化显示。如果地址的符号名未知，或指定了 `-n` 选项，则根据地址族以数字形式打印地址。有关 Internet IPv4 “点分格式” 的更多信息，参见 [inet(3)](../net/inet.3.md)。未指定的或 “通配” 地址和端口显示为 “`*`”。
