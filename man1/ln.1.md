...
符号链接包含它所链接到的文件的名称。对链接执行 [open(2)](../man2/open.2.md) 操作时，将使用被引用的文件。对符号链接执行 [stat(2)](../man2/stat.2.md) 将返回被链接的文件；必须执行 [lstat(2)](../man2/lstat.2.md) 才能获取有关链接本身的信息。可以使用 [readlink(2)](../man2/readlink.2.md) 调用来读取符号链接的内容。符号链接可以跨越文件系统，并且可以指向目录。
...
当该实用程序以 `link` 名称调用时，必须提供恰好两个参数，且两个参数都不能指定目录。在此简单操作模式下不能提供任何选项，它使用传入的两个参数执行 [link(2)](../man2/link.2.md) 操作。
...
[link(2)](../man2/link.2.md), [lstat(2)](../man2/lstat.2.md), [readlink(2)](../man2/readlink.2.md), [stat(2)](../man2/stat.2.md), [symlink(2)](../man2/symlink.2.md), [symlink(7)](../man7/symlink.7.md)
