...
`qlxgbe` 驱动程序支持 IPv4 校验和卸载、IPv4 和 IPv6 的 TCP 与 UDP 校验和卸载、IPv4 和 IPv6 的大段卸载、Jumbo 帧、VLAN 标签、接收端扩展，以及在启用 LRO 时选择硬件或软件 LRO 的能力（默认为硬件 LRO）。有关更多硬件信息，参见 `http://www.qlogic.com/`。

## 硬件

`qlxgbe` 驱动程序支持基于以下芯片组的 10 千兆以太网与 CNA 适配器：
...
`qlxgbe` 设备驱动程序首次出现于 FreeBSD 10.0。

## 作者

`qlxgbe` 驱动程序由 QLogic Corporation 的 David C Somayajulu 编写。
