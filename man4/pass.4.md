...
`pass` 驱动让用户态应用程序能向内核发送 CAM CCB。

由于 `pass` 驱动允许直接访问 CAM 子系统，系统管理员在授予对此驱动的访问权限时应格外谨慎。如果使用不当，此驱动可能允许用户态应用程序使机器崩溃或导致数据丢失。

`pass` 驱动附加到系统中找到的每个 SCSI 和 ATA 设备。由于它附加到每个设备，因此能通用访问 SCSI 和 ATA 设备，并允许用户访问没有关联“标准”外设驱动程序的设备。
