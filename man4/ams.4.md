...
`ams` 驱动为连接到 Apple Desktop Bus（ADB）的鼠标和触控板提供支持，实现了基本和扩展 ADB 鼠标协议。
...
**`dev.ams.%d.tapping`** 在 ADB 触控板上，将此 sysctl 设置为 1 会使触控板上的点按被解释为按钮单击。
...
[adb(4)](adb.4.md), [cuda(4)](cuda.4.md), [pmu(4)](pmu.4.md)

## 历史

`ams` 设备驱动出现于 FreeBSD 8.0。
