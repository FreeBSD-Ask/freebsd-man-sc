...
`cd` 驱动为 SCSI CD-ROM（Compact Disc-Read Only Memory，光盘只读存储器）驱动器提供支持。为使其看起来像常规磁盘，`cd` 驱动会合成一个分区表，其中一个分区覆盖整个 CD-ROM。可以使用 disklabel(8) 修改此分区表，但修改仅在 CD-ROM 卸载之前有效。总体而言，其接口与 [ada(4)](ada.4.md) 和 [da(4)](da.4.md) 中描述的类似。

在引导期间探测 SCSI 适配器时，会扫描 SCSI 总线以查找设备。任何应答为 CDROM（类型 5）或 WORM（类型 4）类型的设备将被“附加”到 `cd` 驱动。在 FreeBSD 2.1 之前，第一个找到的设备将被附加为 `cd0`，下一个为 `cd1`，依此类推。从 FreeBSD 2.1 起，可以指定设备以何种 cd 单元号上线；有关内核配置的详细信息，请参见 [scsi(4)](scsi.4.md)。
...
当由 `cd` 驱动控制的驱动器中的 CD-ROM 被更换时，更换介质的操作会使磁盘标签和内核中保存的信息失效。为防止数据损坏，对设备的所有访问将被丢弃，直到没有更多打开的文件描述符引用该设备。在此期间，所有新的打开尝试都将被拒绝。当没有更多打开的文件描述符引用该设备时，下一次打开将加载该驱动器的新参数集（包括磁盘标签）。
