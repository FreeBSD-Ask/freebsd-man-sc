...
`ada` 驱动提供对实现 ATA 命令协议、通过 CAM 子系统支持的主机适配器连接到系统的直接访问设备的支持。

在配置 ATA 直接访问设备之前，主机适配器也必须单独配置进系统。

## 命令排队

命令排队允许设备并发处理多个事务，通常会重新排序以减少寻道次数和长度。ATA 定义了两种排队类型：TCQ（Tagged Command Queuing，PATA 传统方式）和 NCQ（Native Command Queuing，SATA）。`ada` 设备驱动在受支持时充分利用 NCQ。为确保对距离当前磁头位置较远的介质区域的访问（这些事务可能因持续处理更近的请求而被无限期推迟）能及时完成，在设备连续运行期间每 7 秒发送一个有序事务。

## 缓存效果

许多直接访问设备配备了读缓存和/或写缓存。影响设备缓存的参数在设备 IDENTIFY 数据中报告，可通过 [camcontrol(8)](../man8/camcontrol.8.md) 工具查看和修改。

读缓存用于存储设备发起的预读操作数据以及常用数据。读缓存对用户透明，启用不会产生任何不良影响。大多数带读缓存的设备出厂时即已启用。

写缓存可大幅降低写操作延迟，并允许设备重新组织写入以提高效率和性能。这种性能提升是有代价的。如果设备在缓存中仍包含未提交的写操作时断电，这些写入将丢失。写事务丢失对文件系统的影响是不确定的，可能导致损坏。大多数设备会对写事务进行老化处理，将脆弱性限制在最近报告为完成的少数事务上，但仍然建议启用写缓存设备的系统配备不间断电源（UPS）。`ada` 设备驱动确保在设备最终关闭或意外关机（panic）事件时缓存和介质同步。这确保一旦操作系统报告已停止，即可安全断电。
