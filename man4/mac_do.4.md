...
`mac_do` 策略模块仅对从可配置白名单中特定可执行文件生成的进程产生作用。默认情况下，此白名单仅包含 `/usr/bin/mdo` 可执行文件，有关此程序的更多详情，请参见 mdo(1)。
...
目前，`mac_do` 仅考虑通过 setcred(2) 系统调用请求的凭证转换。此系统调用在很大程度上是为 `mac_do` 创建的，以便它能查看整个凭证转换并决定是否授权，而传统 UNIX 通过连续更改不同部分的方式无法做到。
