...
`mps` 驱动为 Broadcom Ltd./Avago Tech（LSI）Fusion-MPT 2 IT/IR SAS 控制器和 WarpDrive 固态存储卡提供支持。

## 硬件

`mps` 驱动支持以下控制器：

- Broadcom Ltd./Avago Tech（LSI）SAS 2004（4 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SAS 2008（8 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SAS 2108（8 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SAS 2116（16 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SAS 2208（8 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SAS 2308（8 端口 SAS）
- Broadcom Ltd./Avago Tech（LSI）SSS6200 固态存储
...
要为所有 `mps` 驱动实例禁用 MSI 中断，请在 loader.conf(5) 中设置此可调参数：

```sh
hw.mps.disable_msi=1
```

要为特定的 `mps` 驱动实例禁用 MSI 中断，请在 loader.conf(5) 中设置此可调参数：

```sh
dev.mps.X.disable_msi=1
```

要为所有 `mps` 驱动实例禁用 MSI-X 中断，请在 loader.conf(5) 中设置此可调参数：

```sh
hw.mps.disable_msix=1
```

要为特定的 `mps` 驱动实例禁用 MSI-X 中断，请在 loader.conf(5) 中设置此可调参数：
...
当前空闲链帧数存储在 dev.mps.X.chain_free [sysctl(8)](../man8/sysctl.8.md) 变量中。

自启动以来观察到的最低空闲链帧数存储在 dev.mps.X.chain_free_lowwater [sysctl(8)](../man8/sysctl.8.md) 变量中。

自启动以来链帧分配失败的次数存储在 dev.mps.X.chain_alloc_fail [sysctl(8)](../man8/sysctl.8.md) 变量中。这可用于确定是否应增大 max_chains 可调参数以提升性能。

当前活动的 I/O 命令数显示在 dev.mps.X.io_cmds_active [sysctl(8)](../man8/sysctl.8.md) 变量中。
...
自启动以来观察到的最高活动 I/O 命令数存储在 dev.mps.X.io_cmds_highwater [sysctl(8)](../man8/sysctl.8.md) 变量中。

要为所有适配器将设备排除在 `mps` 控制之外，请在 loader.conf(5) 中设置此可调参数：
...
要为特定适配器将设备排除在 `mps` 控制之外，请在 loader.conf(5) 中设置此可调参数：
...
通过在 loader.conf(5) 中使用全局 `hw.mps.debug_level` 和按设备的 `dev.mps.X.debug_level` 可调参数来控制驱动诊断输出。可在运行时使用 [sysctl(8)](../man8/sysctl.8.md) 变量 `dev.mps.X.debug_level` 更改任何适配器的调试级别。
...
[cam(4)](scsi.4.md), [cd(4)](cd.4.md), [ch(4)](ch.4.md), [da(4)](da.4.md), [mpr(4)](mpr.4.md), [mpt(4)](mpt.4.md), [pci(4)](pci.4.md), [sa(4)](sa.4.md), [scsi(4)](scsi.4.md), [targ(4)](targ.4.md), [loader.conf(5)](../man5/loader.conf.5.md), [mpsutil(8)](../man8/mpsutil.8.md), [sysctl(8)](../man8/sysctl.8.md)

## 历史

`mps` 驱动首次出现于 FreeBSD 9.0。

## 作者

`mps` 驱动最初由 Scott Long <scottl@FreeBSD.org> 编写。LSI Corporation、Avago Technologies（前身为 LSI）和 Broadcom Ltd.（前身为 Avago）对其进行了改进和测试。
